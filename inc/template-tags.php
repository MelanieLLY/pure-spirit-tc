...
if (!function_exists('bootstrapBasicCategoriesList')) {
	...
	{
		return sprintf('<span class="categories-icon glyphicon glyphicon-th-list" title="' . __('Posted in', 'pure-spirit') . '"></span> %1$s', $categories_list);
		...
		if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
			...
			echo '<div class="comment-body media">';
				echo '<div class="media-body">';
					_e('Pingback:', 'pure-spirit');
					comment_author_link(); 
					edit_comment_link(__('Edit', 'pure-spirit'), '<span class="edit-link">', '</span>');
				echo '</div>';
			echo '</div>';
		} else {
			echo '<li id="comment-';
			...
			echo '" class="comment-body media">';
				...
				echo '<div class="comment-content media-body">';
					echo '<div class="comment-author vcard">';
						...
						printf(_x('%1$s at %2$s', '1: date, 2: time', 'pure-spirit'), get_comment_date(), get_comment_time());
						echo '</time>';
						echo '</a>';
						// end date-time
						...
						edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit', 'pure-spirit'), '<span class="edit-link">', '</span>');
						...
						if ('0' == $comment->comment_approved) {
							echo '<div class="comment-awaiting-moderation text-warning"> <span class="glyphicon glyphicon-info-sign"></span> ';
								_e('Your comment is awaiting moderation.', 'pure-spirit');
							echo '</div>';
						} //endif;

						// comment author says
						/* translators: %s: Comment author link. */
						printf(__('%s <span class="says">says:</span>', 'pure-spirit'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
					...
					comment_reply_link(array_merge($args, array(
						...
						'reply_text' => '<span class="fa fa-reply"></span> ' . __('Reply', 'pure-spirit'),
						'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'pure-spirit')
					)));
					// end reply link
...
if (!function_exists('bootstrapBasicEditPostLink')) {
	...
	function bootstrapBasicEditPostLink() 
	{
		...
		if ($edit_post_link != null) {
		    $edit_btn = '<a class="post-edit-link btn btn-default btn-xs" href="'.$edit_post_link.'" title="' . __('Edit', 'pure-spirit') . '"><i class="edit-post-icon glyphicon glyphicon-pencil" title="' . __('Edit', 'pure-spirit') . '"></i></a>';
...
if (!function_exists('bootstrapBasicFullPageSearchForm')) {
	...
	function bootstrapBasicFullPageSearchForm() 
	{
		...
		$output .= '<input type="text" name="s" value="' . esc_attr(get_search_query()) . '" placeholder="' . esc_attr_x('Search &hellip;', 'placeholder', 'pure-spirit') . '" title="' . esc_attr_x('Search &hellip;', 'label', 'pure-spirit') . '" class="form-control" />';
		$output .= '</div>';
		$output .= '<div class="col-xs-2">';
		$output .= '<button type="submit" class="btn btn-default">' . __('Search', 'pure-spirit') . '</button>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</form>';
...
if (!function_exists('bootstrapBasicMoreLinkText')) {
	...
	{
		return __('Continue reading <span class="meta-nav">&rarr;</span>', 'pure-spirit');
...
if (!function_exists('bootstrapBasicPostOn')) {
	...
	function bootstrapBasicPostOn() 
	{
		...
		printf(__('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'pure-spirit'),
			...
			sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
				...
				esc_attr(sprintf(__('View all posts by %s', 'pure-spirit'), get_the_author())),
...
if (!function_exists('bootstrapBasicTagsList')) {
	...
	{
		return sprintf('<span class="tags-icon glyphicon glyphicon-tags" title="' . __('Tagged', 'pure-spirit') . '"></span>&nbsp; %1$s', $tags_list);
